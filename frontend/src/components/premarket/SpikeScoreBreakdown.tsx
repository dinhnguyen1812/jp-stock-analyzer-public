...
const SpikeScoreBreakdown: React.FC<Props> = ({ spikeInfo }) => {
  ...
  const breakdown: { label: string; points: number; value?: any }[] = [
    { label: "First Close↑High", value: spikeInfo.first_day_close_near_high ? "Yes" : "No", points: spikeInfo.first_day_close_near_high ? 10 : 0 },
    { label: "First spike pct", value: spikeInfo.first_spike_pct + "%", points: spikeInfo.first_spike_pct >= 100 ? 20 : spikeInfo.first_spike_pct >= 40 ? 10 : 5 },
    { label: "Days since spike", value: spikeInfo.days_since_spike, points: spikeInfo.days_since_spike <= 5 ? 25 : spikeInfo.days_since_spike <= 10 ? 15 : 5 },
    { label: "Respikes", value: spikeInfo.number_of_respikes, points: Math.min(spikeInfo.number_of_respikes ?? 0, 1) * 5 },
    { label: "Current Drop↓High", value: spikeInfo.drop_from_high_pct + "%", points: spikeInfo.drop_from_high_pct >= 40 ? 25 : spikeInfo.drop_from_high_pct >= 20 ? 18 : spikeInfo.drop_from_high_pct >= 10 ? 10 : 5 },
    { label: "Current Close↓Low", value: spikeInfo.last_day_close_near_low ? "Yes" : "No", points: spikeInfo.last_day_close_near_low ? 15 : 0 }
  ];
